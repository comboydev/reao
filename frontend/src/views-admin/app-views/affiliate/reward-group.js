...
export const RewardGroup = () => {
	...
	useEffect(()=>{
		AdminService.adminGetRewardGroup()
		.then(res=>{
	...
	const handleSave = () => {
		...
		.then(values => {
			...
			try {
				if (mode === ADD) {
					msg = "新規追加しました!"
					promise = AdminService.adminCreateRewardGroup(values)
				}
				else {
					msg = "更新しました!"
					promise = AdminService.adminUpdateRewardGroup(selectedRow._id, values)
				}
				promise.then(res => {
					setSubmit(false);
	...
	const handleDelete = (id) => {
		setSubmit(true);
		AdminService.adminDeleteRewardGroup(id)
		.then(res => {
			setSubmit(false);	
