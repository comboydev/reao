...
const  Profile = (props) => {
	...
	useEffect(async () => {
		try {
			let res = await AdminService.adminGetUserOne(props.match.params.id)
			setUserData(res.data)
	
			res = await UserService.getPartners(props.match.params.id)
			setTear(res.data);

			res = await AdminService.adminGetRewardGroup()
	...
	const handleChangeRewardGroup = async (e) => {
		let payload = {
			id: userData._id,
			rewardGroup: e.target.value,
		}
		try {
			let res = await UserService.updateRewardGroup(payload)
			setUserData(res.data)
		} catch (err) {
			...
			<PageHeaderAlt  cssClass="bg-primary" overlap>
				<Col sm={24} md={18} lg={16} xl={12} className="ml-sm-5">
					<div className="d-sm-flex">
						<div className="rounded p-2 mx-auto text-center" style={{'maxWidth': `${avatarSize}`}}>
							<Avatar shape="circle" size={avatarSize} src={ userData.avatar } />
						</div>
