...
import { _PAYMENT_TYPE } from "constants/AppConstant";
...
export default function PurchaseRequest(props) {
            ...
            >
                <div className="text-center">
                    ...
                    <Row gutter={20}>
                        ...
                        <Col xs={12} className="mb-2">
                            <p className="text-left">{_PAYMENT_TYPE[orderForm.getFieldValue().paymentType].type}</p>
                        </Col>
                    </Row>
                </div>
            </Modal>
    ...
    const onSubmit = () => {
        ...
        let req = {
            ownershipID: coin.ownership._id,
            buyerID: UserService.getCurrentUser()._id,
        ...
        setSubmit(true);
        setShowModal(false);
        UserService.orderOwnership(req)
        .then(res => {
            setSubmit(false);
                    ...
                    <div className="step2">
                        ...
                        <div className="w-full mt-16 mb-16 flex items-center justify-center">
                            <Form.Item name="paymentType">
                                <Radio.Group>
                                    {
                                        _PAYMENT_TYPE.map((d, i)=>
                                            <Radio value={d.value} key={i}>{d.type}</Radio>
                                        )
                                    }
                                </Radio.Group>
                            </Form.Item>
