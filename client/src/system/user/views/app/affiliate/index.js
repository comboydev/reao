...
const columns = [
	...
	},
	{
		...
		key: '_id',
		dataIndex: '_id',
		render: _id => <Tag color="blue" >{_id.toUpperCase()}</Tag>,
	},
];
...
export default function Partner(){
	...
	const _introducer = `${window.location.origin}/register?introducer=${user._id}`;
	...
	useEffect(() => {
		async function fetchData() {
			try {
				let res = await userProfile.getPartners(user._id);
				setTear(res.data);

				res = await userProfile.getPersonalInfo(user._id);
