...
import adminCoin from 'api/admin/coin';
...
const CoinForm = ({ mode = CREATE_MODE, param }) => {
	...
	const onSubmit = () => {
		...
		.then(async (values) => {
			...
			try {
				if (mode === EDIT_MODE) {
					await adminCoin.update(token.uri, values);
					history.push(`/admin/coins/detail/${token.tokenId}`);
				} else if(mode === CREATE_MODE) {
					// Add Coin
					const { data } = await adminCoin.create(values);
					try{
						...
						console.log(err);
						setLoading(false);
						await adminCoin.deleteBatch([data.coin.id]);
